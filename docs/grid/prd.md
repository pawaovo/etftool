...
主页面原型展示参考：`docs\grid\1.png`

*开发者注：主界面是基于`150plan.html`页面，样式UI一致。*
...
*开发者注：该界面是基于`150plan.html`和`grid.html`页面，样式UI一致。*
...
原型设计可参考：`docs\grid\2.png`

分为两部分：上半部分的网格策略参数和下半部分的网格策略表格。

网格策略参数包括：标的类型、最小报价单位、最小交易单位、基准价、每份金额、最低价、小网步长、中网步长、大网步长、档位加码系数、金额加码系数、保留利润系数。该部分默认根据拿到的源数据处理后进行展示，支持通过“修改”按钮，修改各参数，在下方的网格策略表格中生成对应的表格信息。
...
- **UI一致性**：页面 (`grid.html`和`grid-detail.html`) 的整体风格（颜色、字体、布局、卡片样式、交互反馈等）应参考 `150plan.html`，保持项目视觉统一。尽可能复用 `plan-data.css` 和 `styles/etf-cards.css` 中的相关样式，但是也需要保证新页面UI样式规范，功能、显示的正常。
